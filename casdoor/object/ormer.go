...
func InitConfig() {
	...
	fmt.Printf("DEBUG: driverName = %q\n", conf.GetConfigString("driverName"))
	fmt.Printf("DEBUG: dbname = %q\n", conf.GetConfigString("dbname"))
	fmt.Printf("DEBUG: dataSourceName = %q\n", conf.GetConfigDataSourceName())

	// Store config values to use in InitAdapter
	driverName := conf.GetConfigString("driverName")
	dbname := conf.GetConfigString("dbname")
	dataSourceName := conf.GetConfigDataSourceName()

	fmt.Printf("DEBUG: Stored values - driverName: %q, dbname: %q\n", driverName, dbname)

	InitAdapterWithValues(driverName, dataSourceName, dbname)
	CreateTables()
}

func InitAdapterWithValues(driverName, dataSourceName, dbname string) {
	fmt.Printf("DEBUG: InitAdapterWithValues called with - driverName: %q, dbname: %q\n", driverName, dbname)
	...
	fmt.Printf("DEBUG: Before createDatabase check - dbname: %q\n", dbname)
	if createDatabase {
		fmt.Printf("DEBUG: Creating database with dbname: %q\n", dbname)
	...
	fmt.Printf("DEBUG: Before NewAdapter call - dbname: %q\n", dbname)
	var err error
...
func NewAdapter(driverName string, dataSourceName string, dbName string) (*Ormer, error) {
	fmt.Printf("DEBUG: NewAdapter called with - driverName: %q, dbName: %q\n", driverName, dbName)
	...
	fmt.Printf("DEBUG: NewAdapter struct values - driverName: %q, dbName: %q\n", a.driverName, a.dbName)
...
func refineDataSourceNameForPostgres(dataSourceName string) string {
	...
	// Replace dbname with postgres for database creation
	reg := regexp.MustCompile(`dbname=[^ ]+\s*`)
	result = reg.ReplaceAllString(result, "dbname=postgres")

	// Add sslmode=disable if not present
	if !strings.Contains(result, "sslmode=") {
		result = result + " sslmode=disable"
	}
...
func (a *Ormer) open() error {
	fmt.Printf("DEBUG: Ormer.open() called with - driverName: %q, dbName: %q\n", a.driverName, a.dbName)

	var dataSourceName string

	// For PostgreSQL, the database name should be included in the connection string
	if a.driverName == "postgres" {
		// Add dbname parameter to the PostgreSQL connection string
		if strings.Contains(a.dataSourceName, "dbname=") {
			// If dbname already exists, replace it
			re := regexp.MustCompile(`dbname=[^\s]*`)
			dataSourceName = re.ReplaceAllString(a.dataSourceName, "dbname="+a.dbName)
		} else {
			// If dbname doesn't exist, append it
			dataSourceName = a.dataSourceName + " dbname=" + a.dbName
		}

		// Add sslmode=disable to avoid SSL issues with local PostgreSQL
		if !strings.Contains(dataSourceName, "sslmode=") {
			dataSourceName = dataSourceName + " sslmode=disable"
		}
	} else {
		// For MySQL and other databases, append dbName directly
		dataSourceName = a.dataSourceName + a.dbName
	}

	fmt.Printf("DEBUG: dataSourceName constructed: %q\n", dataSourceName)

	engine, err := xorm.NewEngine(a.driverName, dataSourceName)
	if err != nil {
		fmt.Printf("DEBUG: xorm.NewEngine error: %v\n", err)
		return err
	}
